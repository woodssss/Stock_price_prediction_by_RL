...
mem_size = 500
RL = DQN(lr=0.00001, gamma=0.97, state_size=windows_size, mem_size=mem_size)
...
for i in range(e_num):
    ...
    for t in range(start_time,len(data)-1):
        ...
        reward = 0
        if action == 1 and 201<t<len(data)-1:  # buy
            RL.buyrecord.append(data[t])
            #reward = -data[t]
        elif action == 2 and len(RL.buyrecord) > 0 and 201<t<len(data)-1:  # sell
            bought_price = RL.buyrecord.pop(0)
            reward = data[t] - bought_price
            total_profit += data[t] - bought_price
            RL.trasition_store(s, action, reward, sp, t)
        elif t==len(data)-2:
